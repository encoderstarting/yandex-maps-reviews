...
Текущий результат включает рабочую аутентификацию, интерфейс, окружение Laravel Sail, доменные таблицы, защищённый API организаций и изолированный парсер публичных страниц Яндекс Карт. API пока не запускает парсер: фоновый worker и сохранение результата будут реализованы отдельным этапом. Подробный план находится в [`task.md`](task.md).
...
Подключённая организация получает статус `pending`. Он не изменится автоматически, пока отдельным этапом не будут добавлены queue worker и сохранение результата парсинга.
