...
| `POST /api/v1/organizations/{id}/sync` | Повторный запуск синхронизации |
...
Поддерживаются HTTPS-ссылки карточек вида `/maps/org/{slug}/{id}` и `/maps/org/{id}` на доменах `yandex.ru` и `yandex.com`. Параметры запроса и завершающий слеш не влияют на проверку дублей; ссылка на другой раздел Карт отклоняется до создания организации.
...
Повторную синхронизацию можно запустить после любого конечного статуса. Одновременно может выполняться только одна задача для организации: повторный запрос во время активной Job возвращает её текущий запуск и не создаёт дубль. Временные ошибки повторяются через 60, 300 и 900 секунд. Отзывы сохраняются через `upsert` по внешнему ID, поэтому повторный импорт не создаёт дубли. Новый снимок рейтинга и счётчиков сохраняется только при их изменении.
